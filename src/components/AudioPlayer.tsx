...
export const AudioPlayer: React.FC<{ onOpenEpisode?: (episode: any) => void }> = ({ onOpenEpisode }) => {
  ...
  return (
    <aside
      ...
      className="fixed bottom-0 left-0 right-0 z-50 bg-[#0e111a]/95 backdrop-blur-xl border-t border-zinc-800 text-zinc-100 shadow-2xl transition-all duration-200"
    >
      ...
      {isMinimized ? (
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={togglePlay}
              className="w-9 h-9 rounded-full bg-red-600 hover:bg-red-500 text-white flex items-center justify-center transition-transform active:scale-95 cursor-pointer shadow-md shadow-red-950/40"
              title={isPlaying ? 'Pause' : 'Play'}
            >
              {isPlaying ? <Pause className="w-4 h-4 fill-current" /> : <Play className="w-4 h-4 fill-current ml-0.5" />}
            </button>
            <div className="min-w-0 cursor-pointer" onClick={() => onOpenEpisode?.(currentEpisode)}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          ...
          <div className="flex items-center justify-between gap-3 sm:gap-6">
            ...
            <div className="flex items-center gap-3 min-w-0 w-1/4 max-w-xs">
              ...
              >
                <img
                  src={currentEpisode.imageUrl || "/assets/cover.jpg"}
